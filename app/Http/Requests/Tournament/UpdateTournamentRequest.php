...
class UpdateTournamentRequest extends FormRequest
{
    ...
    public function ensure_uniqueness()
    {
       ...
       $this->ensureIsNotRateLimited('tournament-update', 5, 'english_name');

       $user = auth()->user();

       $user->load('tournaments');

       if(!$user->tournaments->where('id', '!=', request()->tournament->id)->where('name_en', $this->english_name)->isEmpty()){
           throw ValidationException::withMessages([
               'english_name' => __('This tournament\'s english name already exists')
           ]);
       }
        if(!$user->tournaments->where('id', '!=', request()->tournament->id)->where('name_ar', $this->arabic_name)->isEmpty()){
           throw ValidationException::withMessages([
