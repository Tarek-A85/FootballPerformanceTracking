...
<div class="container mx-auto px-1 ">
    <div class="flex justify-between items-center py-8">
        ...
        <!-- Page Header -->
        <div class="truncate">
        <h1 class="text-3xl font-bold truncate ">{{ __('Your Overall Schedule') }}</h1>
        </div>
    ...
    <div class="mt-2">
        ...
        <div>
            <a href="{{ route('schedule.index') }}" class="bg-gray-400 p-2 rounded-md">
              {{ __('Reset') }}
           </a>
           </div>
           </div>
    ...
    <div class="rounded border-2 bg-gray-400 mt-8 w-full md:w-3/4 ">
        ...
        <div class="p-4">
            <p class="text-xl font-bold mt-8">  {{ Carbon\Carbon::parse($key)->translatedFormat('l')}} 
       ...
       <a href="{{ route('matches.show', ['match' => $event]) }}">
        <div class="grid grid-cols-3 rounded border-2 mt-5 p-5 bg-white w-full" >
            <!-- Team -->
          ...
          >
           ...
           @endif
           <p >{{ $event->tournament['name_'. app()->getLocale()] }}</p>
        ...
        <div class="flex justify-between rounded border-2 mt-5 bg-white w-full p-5">
            <p class="text-xl font-bold">{{$event->training_type['name_' . app()->getLocale()]}} - {{ __('Training') }}
                {{ $event->trainable_type === 'team' ? __('with ') .  $event->trainable['name_' . app()->getLocale()] : '' }} <small> ( {{ __('for :hours hours and :minutes minutes', ['hours' => (floor($event->minutes / 60)), 'minutes' => round(($event->minutes % 60))]) }} )</small>
        </p>
